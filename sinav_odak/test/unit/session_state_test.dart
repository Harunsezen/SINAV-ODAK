...
void main() {
  ...
  group('inBreak', () {
    ...
    test('uzatma sayısı state içinde taşınıyor', () {
      ...
      expect(
        (breakState(extensionsUsed: 0) as SessionInBreak).extensionsUsed,
        0,
      );
      expect(
        (breakState(extensionsUsed: 1) as SessionInBreak).extensionsUsed,
        1,
      );
      expect(
        (breakState(extensionsUsed: 2) as SessionInBreak).extensionsUsed,
        2,
      );
    });
  ...
  group('remainingSeconds aşağı yuvarlıyor', () {
    ...
    test(
      '999 ms -> 0 sn',
      () => expect(withRemaining(999).remainingSeconds, 0),
    );
